...
template <typename Matrix, typename Vector>
int dtrsv( const int Side, const int UpperLower, const int TransA, const int Diag, const Matrix& A, Vector& b )
{
   const int m = A.mRows();
   const int n = A.nCols();
   ...
   if (Side == SideLeft)
   {
      ...
      if (UpperLower == TriLower)
      {
         for (int j = 0; j < m; ++j)
         {
            ...
            if (b_j != 0.0)
            {
               if (Diag == NotUnitDiagonal) b_j = ( b(j) /= A(j,j) );
      ...
      else
      {
         for (int j = m-1; j >= 0; --j)
         {
            ...
            if (b_j != 0.0)
            {
               if (Diag == NotUnitDiagonal) { b(j) /= A(j,j); b_j = b(j); }
   ...
   {
      fprintf(stderr,"Right looking matrix problem not supported in par_dtrsv\n"); exit(1);
   }
}

template <typename Matrix, typename Vector>
int par1_dtrsv( const int Side, const int UpperLower, const int TransA, const int Diag, const Matrix& A, Vector& b )
{
   const int m = A.mRows();
   const int n = A.nCols();
   ...
   {
      fprintf(stderr,"Right looking matrix problem not supported in par_dtrsm\n"); exit(1);
   }
}

template <typename Matrix, typename Vector>
int par2_dtrsv( const int Side, const int UpperLower, const int TransA, const int Diag, const Matrix& A, Vector& b, const int blockSize = 32)
{
   typedef typename Vector::value_type value_type;

   const int m = A.mRows();
   const int n = A.nCols();
   const int nblocks = (m + (blockSize-1)) / blockSize;
   ...
   if (Side == SideLeft)
   {
      ...
      if (UpperLower == TriLower)
      {
         #pragma omp parallel
         {
            ...
            {
               const int k0 = kb * blockSize;
               const int k1 = std::min( k0 + blockSize, m );

               // Solve the small triangular system [k0,k1) starting at A(k0,k0)
               #pragma omp single
               {
                  Matrix At( const_cast<typename Matrix::value_type*>(&A(k0,k0)), (k1-k0), (k1-k0), A.lDim() );
                  Vector bt( &b(k0), (k1-k0) );

                  dtrsv( SideLeft, TriLower, TransA, Diag, At, bt );
               }

               // Now update the tiles under this diagonal.

               #pragma omp for
               for (int ib = kb+1; ib < nblocks; ++ib)
               {
                  const int i0 = ib * blockSize;
                  const int i1 = std::min( i0 + blockSize, m );

                  for (int j = k0; j < k1; ++j)
                  {
                     const value_type b_j = b(j);
                     if (b_j != 0.0)
                     {
                        #pragma ivdep
                        for (int i = i0; i < i1; ++i)
         ...
         #pragma omp parallel
         {
            for (int kb = nblocks-1; kb >= 0; kb--)
            {
               ...
               #pragma omp single
               {
                  ...
                  dtrsv( SideLeft, TriUpper, TransA, Diag, At, bt );
               }

               // Now update the tiles above this diagonal block.
               #pragma omp for
...
/// Mimic distributed solver with cylic assignment of threads to block rows.
template <typename Matrix, typename Vector>
int par3_dtrsv( const int Side, const int UpperLower, const int TransA, const int Diag, const Matrix& A, Vector& b, const int blockSize = 32)
{
   typedef typename Vector::value_type value_type;
      ...
      if (UpperLower == TriLower)
      {
         #pragma omp parallel
         {
            ...
            for (int kb = 0; kb < nblocks; ++kb)
            {
               ...
               // Solve the small triangular system [k0,k1) starting at A(k0,k0)
               // if I own this block-row.
               if ( kb % n_threads == thread_id)
               {
                  Matrix At( const_cast<typename Matrix::value_type*>(&A(k0,k0)), (k1-k0), (k1-k0), A.lDim() );
                  Vector bt( &b(k0), (k1-k0) );
               ...
               // Everyone waits for the diagonal solve to finish.
               #pragma omp barrier

               // Now update the tiles under this diagonal.

               for (int ib = kb+1; ib < nblocks; ++ib)
                  if ( ib % n_threads == thread_id )
                  {
                     const int i0 = ib * blockSize;
                     const int i1 = std::min( i0 + blockSize, m );
               ...
               // Prevent a thread from jump to the next sweep before all the blocks are finished.
               // This isn't necessary if using tasks with a dependency!
               #pragma omp barrier
            }
         } // end parallel
      }
      ...
      else
      {
         #pragma omp parallel
         {
            ...
            for (int kb = nblocks-1; kb >= 0; --kb)
            {
               ...
               // Now update the tiles above this diagonal.

               for (int ib = thread_id; ib < kb; ib += n_threads)
               {
                  const int i0 = ib * blockSize;
                  const int i1 = std::min( i0 + blockSize, m );
...
template <typename Matrix, typename Vector>
int ludec_tile_task( Matrix& A, Vector& P, const int blockSize = 32 )
{
   int m = A.mRows();
   int n = A.nCols();
   int ldim = A.lDim();
   ...
   const int nb = (m + blockSize - 1) / blockSize;
   char Ab[nb][nb], Pb[nb];
   //if ( (m + blockSize - 1) / blockSize > maxBlocks )
   //{
   //   fprintf(stderr,"Too many blocks requested %d\n", maxBlocks);
   //   return -1;
   //}
   ...
   // Loop over the diagonal tiles.
   for (int k0 = 0, kb = 0; k0 < n; k0 += blockSize, ++kb)
   {
      // First/last colums of this tile.
      const int k1 = std::min( n, k0 + blockSize);
      ...
      #pragma omp task depend( inout: Ab[kb][kb] ) \
                       depend( out: Pb[kb] ) \
                       firstprivate( Akk, Pk )
      ludec_naive( Akk, Pk );

      // Solve the upper tiles on this row.
      for (int j0 = k1, jb = kb+1; j0 < n; j0 += blockSize, ++jb)
      #pragma omp task depend( in: Ab[kb][kb], Pb[kb] ) \
                       depend( inout: Ab[kb][jb] ) \
                       firstprivate( j0, k0, k1, Akk, Pk )
      {
         const int j1 = std::min(n, j0 + blockSize);
      ...
      // Pivot the left tiles on this row.
      // This and the final pivot update are leaf tasks.
      for (int j0 = 0, jb = 0; j0 < k0; j0 += blockSize, ++jb)
      #pragma omp task depend( inout: Ab[kb][jb] ) \
                       depend( in: Pb[kb] ) \
                       firstprivate( j0, k0, k1, Pk )
      {
         const int j1 = std::min(n, j0 + blockSize);
      ...
      // Done with the local pivot. Reset to a global index.
      #pragma omp task depend( inout: Pb[kb] ) \
                       firstprivate( k0, k1 ) shared( P )
      for (int i = k0; i < k1; ++i)
         P(i) += k0;

      // Solve for the lower tiles on this column
      for (int i0 = k1, ib = kb+1; i0 < m; i0 += blockSize, ++ib)
      {
         const int i1 = std::min(m, i0 + blockSize);
         ...
         // Solve the lower tile matrices L(i0:i1,k0:k1) U_kk = A(i0:i1,k0:k1)
         #pragma omp task depend( in: Ab[kb][kb] ) \
                          depend( inout: Ab[ib][kb] ) \
                          firstprivate( Akk, Lik )
         dtrsm( SideRight, TriUpper, NoTranspose, NotUnitDiagonal, Akk, Lik );

         // Update the trailing submatrix tiles ... A_ij -= Lik Ukj
         for (int j0 = k1, jb = kb+1; j0 < n; j0 += blockSize, ++jb)
         #pragma omp task depend( in: Ab[ib][kb], Ab[kb][jb] ) \
                          depend( inout: Ab[ib][jb] ) \
                          firstprivate( i0, j0, k0, i1, k1, Akk, Lik )
         {
            const int j1 = std::min(n, j0 + blockSize);
...
int main (int argc, char * argv[])
{
   ...
   if (verify)
      fprintf(stderr,"Verifying solution\n");
   ...
   while (true)
   {
      ...
      for (int iter = 0; iter < niters; iter++)
      {
         ...
         if (method == 0)
            ...
            ierr = ludec_tile_task( LU, P, blockSize );
   ...
   printf("LU Factorization took (sec): %d %f %f %d %s %d\n", n, tCalc / niters, tCopy / niters, niters, methods[method].c_str(), blockSize);
   ...
   if (verify)
   {
      ...
      #pragma omp parallel for reduction(+: err2, ref2)
      for (int i = 0; i < n; ++i)
         for (int j = 0; j < n; ++j)
         {
      ...
      //if (!passed)
      {
         ...
         dgemm( NoTranspose, NoTranspose, 1.0, A, x, 0.0, b0);
         ...
         while (true)
         {
            ...
            {
               VectorType<double> bv(_b, n);

               for (int i = 0; i < n; ++i) bv(i) = b0(i,0);
               ...
               if (false)
               {
                  ...
                  int one = 1;
                  int info = 0;
                  dgetrs_ (const_cast<char*>(TRANS), &n, &one, LU.getRawPointer(), &n, P.getRawPointer(), bv.getRawPointer(), &n, &info );
               }
               else
               {
                  dlaswp( b, P );
                  ...
                  //dtrsv( SideLeft, TriLower, NoTranspose,    UnitDiagonal, LU, bv );
                  //dtrsv( SideLeft, TriUpper, NoTranspose, NotUnitDiagonal, LU, bv );

                  //par1_dtrsv( SideLeft, TriLower, NoTranspose,    UnitDiagonal, LU, bv );
                  //par1_dtrsv( SideLeft, TriUpper, NoTranspose, NotUnitDiagonal, LU, bv );

                  par2_dtrsv( SideLeft, TriLower, NoTranspose,    UnitDiagonal, LU, bv );
                  par2_dtrsv( SideLeft, TriUpper, NoTranspose, NotUnitDiagonal, LU, bv );

                  //par3_dtrsv( SideLeft, TriLower, NoTranspose,    UnitDiagonal, LU, bv );
                  //par3_dtrsv( SideLeft, TriUpper, NoTranspose, NotUnitDiagonal, LU, bv );
               }
            }
         ...
         printf("Triangular solver took %f (ms) %d\n", 1000.*tSolve/niters, niters);
